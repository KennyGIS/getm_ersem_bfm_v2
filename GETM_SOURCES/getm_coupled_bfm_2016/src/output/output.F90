...
! !USES:
   ...
   use variables_2d, only: calc_taubmax
...
!JM we want this for BFM
#ifdef BFM_GOTM
   if (save_init .and. n.eq.firstN-1) then
      write_2d = .true.
      write_3d = .true.
   end if
#endif
...
#ifdef GETM_BIO
#ifdef BFM_GOTM
               call restart_file_bio(WRITING,BINARY,RESTART,julianday)
#else
               if (bio_calc) then
...
#ifdef BFM_GOTM
           LEVEL3 'bio_calc= ',bio_calc
           LEVEL3 'hotstart_bio= ',hotstart_bio
           if (bio_calc.and.hotstart_bio) then
              call restart_file_bio(READING,NETCDF,RESTART,jd)
           else
...
#ifdef GETM_BIO
#ifdef BFM_GOTM
             if (bio_calc .and. hotstart_bio) call restart_file_bio(READING,BINARY,RESTART,jd) !AN why this second call?
#endif
