...
#ifdef BFM_GOTM
   use coupling_getm_bfm,ONLY: tracer_type,unlabeled_var_index
...
#ifdef BFM_GOTM
      do m=1,numc
         err=assign_river_inputs_to_bio_states(ncid,real_river_name(n),n,m,ll)
!if (n==96) then    !96 is the Rhine
!LEVEL1 'ncdf_rivers.f90, initialising'
!LEVEL1 'rriver,numc',rriver,numc
!LEVEL1 'n,m',n,m
!LEVEL1 'rivername real_river_name ',river_name(n),real_river_name(n)
!LEVEL1 'll',ll
!endif
         select case (ll)
           case (0)
            ...
            case (-2) 
              ...
              river_bio(n,m)=-9999.0
            case default
              bio_id(n,m)=ll;      ! if bio_id>0 : forcing
              river_bio(n,m)=0.0;  ! otherwise min_max functions will be used.
          end select
        end do
!if (n==96) then    !96 is the Rhine
!stop
...
!if (nn==96) then    !96 is the Rhine
!LEVEL1 'ncdf_rivers.f90'
!LEVEL1 'rriver,numc',rriver,numc
!LEVEL1 'nn,j',nn,j
!LEVEL1 'rivername',river_name(nn)
!LEVEL1 'bio_id',bio_id(nn,j)
!LEVEL1 'river_bio',river_bio(nn,j)
!endif
            end do
