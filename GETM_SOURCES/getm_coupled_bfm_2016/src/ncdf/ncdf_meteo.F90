...
!           need airp(ihl+1,:) and/or airp(:,jhl+1) for periodic velocity points
         ...
         if (.not. first) then
            if (airp_id.gt.0) d_airp = airp_new - airp_old
            if (calc_met) then
               if (tcc_id.gt.0) d_tcc = tcc_new - tcc_old
               if (u10_id.gt.0) d_u10 = u10_new - u10_old
               if (v10_id.gt.0) d_v10 = v10_new - v10_old
               if (interpolate_meteo) then
                  if (t2_id .gt.0) d_t2  = t2_new  - t2_old
                  if (hum_id.gt.0) d_hum = hum_new - hum_old
               end if
            else
               if (swr_id.gt.0) d_swr = swr_new - swr_old
            end if
            if (fwf_method.eq.2 .or. fwf_method.eq.3) then
               if (precip_id.gt.0) d_precip = precip_new - precip_old
            end if
      ...
      if (airp_id.gt.0) airp = airp_new + d_airp*deltm1*t_minus_t2
      if (calc_met) then
         if (tcc_id.gt.0) tcc = tcc_new + d_tcc*deltm1*t_minus_t2
         if (u10_id.gt.0) u10 = u10_new + d_u10*deltm1*t_minus_t2
         if (v10_id.gt.0) v10 = v10_new + d_v10*deltm1*t_minus_t2
         if (interpolate_meteo) then
            if (t2_id .gt.0) t2  = t2_new  + d_t2 *deltm1*t_minus_t2
            if (hum_id.gt.0) hum = hum_new + d_hum*deltm1*t_minus_t2
         end if
      else
         if (swr_id.gt.0) swr = swr_new + d_swr*deltm1*t_minus_t2
      end if
      if (fwf_method.eq.2 .or. fwf_method.eq.3) then
         if (precip_id.gt.0) precip = precip_new + d_precip*deltm1*t_minus_t2
      end if
...
!              changed indices - kb 2014-12-15
      ...
      if (fwf_method .eq. 2) then
         evap_id   = ncdf_meteo_inq_varid(ncid,name_evap)
      end if
      if (fwf_method .eq. 2 .or. fwf_method .eq. 3) then
         precip_id = ncdf_meteo_inq_varid(ncid,name_precip)
      end if
...
!KBKSTDERR 'Taking hum as wet bulb temperature'
         ...
         if (t2_id.eq.-1 .and. tcc_id.eq.-1 .and. hum_id.eq.-1) then
            ...
            LEVEL4 '         Stresses will be calculated with constant or neutral drag coefficient.'
            constant_cd = .true.
...
!-----------------------------------------------------------------------
   ...
   call add_secs(junit,sunit,nint(met_times(indx)),met_jul,met_secs)
   call write_time_string(met_jul,met_secs,met_str)
   call write_time_string()
