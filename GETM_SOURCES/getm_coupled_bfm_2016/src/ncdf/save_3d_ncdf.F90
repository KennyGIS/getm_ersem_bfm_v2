...
! !USES:
   ...
   use variables_3d, only: velx3d,vely3d,w,velxAdv,velyAdv
...
!  avg. velocities
   if (u_id .ne. -1) then
      err = nf90_put_var(ncid,u_id,velxAdv(_2D_W_),start,edges)
      if (err .NE. NF90_NOERR) go to 10
   end if
   if (v_id .ne. -1) then
      err = nf90_put_var(ncid,v_id,velyAdv(_2D_W_),start,edges)
      if (err .NE. NF90_NOERR) go to 10
   end if
...
!JM              ffb=ccb3d_out(k,:,:,1)
...
#ifdef INCLUDE_DIAGNOS_PRF
      allocate(ws_prf(I2DFIELD,0:nprf),stat=k)
      start(1) = 1
      start(2) = 1
      start(3) = 1
      start(4) = n3d
      ...
      do n=stPRFDiagS, stPRFDiagE
         if ((var_ids(n) /= 0)  .and. var_ave(n) ) then   !BFM 
           ! Only values to be stored are copied in the diagnostic 4d-array
           k=k+1
!JM           call cnv_3d(imin,jmin,imax,jmax,kmin,nprf,az,ccb3d_prf(k,:,:,:),bio_missing, &
!JM                  imin,imax,jmin,jmax,0,nprf,ws_prf)
           call cnv_3d(imin,jmin,imax,jmax,kmin,nprf,az,ccb3d_prf(:,:,:,k),bio_missing, &
                  imin,imax,jmin,jmax,0,nprf,ws_prf)
           err = nf90_put_var(ncid, var_ids(n), ws_prf(_2D_W_,0:nprf), start=start, count=edges)   !BFM
           if (err .NE.  NF90_NOERR) then
              string='when saving averaged diagnostic vars:'//trim(var_names(n))
              call netcdf_error(err, 'save_3d_ncdf',trim(string))
           endif
...
!JM                  imin,imax,jmin,jmax,0,nprf,ws_prf)
           ...
           err = nf90_put_var(ncid, var_ids(n), ws_prf(_2D_W_,0:nprf), start=start, count=edges)   !BFM
           if (err .NE.  NF90_NOERR) then
              STDERR 'size ws_prf=',size(ws_prf),xlen*ylen*(nprf+1),xlen,ylen
...
#else
      do n=1,numc
!JM         call cnv_3d(imin,jmin,imax,jmax,kmin,kmax,az,cc3d(n,:,:,:), &
!JM                     bio_missing,imin,imax,jmin,jmax,0,kmax,ws)
