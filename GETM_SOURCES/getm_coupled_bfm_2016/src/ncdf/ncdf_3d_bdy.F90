...
! !PRIVATE DATA MEMBERS:
   ...
   REALTYPE,dimension(:,:),pointer,contiguous :: uu_bdy,uu_bdy_new,d_uu_bdy
   REALTYPE,dimension(:,:),pointer,contiguous :: vv_bdy,vv_bdy_new,d_vv_bdy
   REALTYPE,dimension(:,:),pointer,contiguous :: S_bdy,S_bdy_new,d_S_bdy
   REALTYPE,dimension(:,:),pointer,contiguous :: T_bdy,T_bdy_new,d_T_bdy
...
#ifdef BFM_GOTM
   REAL*4,  dimension(2)                     :: att_wrk
   REAL*4                                    :: att_val
   REALTYPE, allocatable, dimension(:,:,:)   :: cc3d_bdy_old, cc3d_bdy_new
   REAL*4,  allocatable, dimension(:,:,:)    :: cc3d_bdy_wrk
   REALTYPE,  allocatable, dimension(:,:)      :: cc3d_bdy_tmp
...
! !LOCAL VARIABLES:
   ...
   logical                   :: dims_ready
#ifdef BFM_GOTM
   integer                   :: bio_bs,i,ib,ibnl,k,l,m,ll,local_ncid
     ...
     do ib=1,n_bio
       if (cc3d_bdy_max(ib)>_ZERO_)  then
         if (.not.limit_all_bio.and.i==0) LEVEL4 &
          "next limits are used only for missing values in boundary timeseries in case of (complete) missing boundaries"  
         write(units, &
...
#ifdef BFM_GOTM
      if ( bio_calc) then 
!LEVEL1 'init_3d_bdy_ncdf. bio_bs,kmax,nsbv',bio_bs,kmax,nsbv
        ...
        allocate(cc3d_bdy_tmp(0:kmax,1:nsbvl),stat=rc)
...
#ifdef BFM_GOTM
         cc3d_bdy_old=cc3d_bdy_wrk  
         cc3d_bdy_new=cc3d_bdy_wrk   !JM added
#endif
...
! !LOCAL VARIABLES:
   ...
   REALTYPE,dimension(:,:),pointer,contiguous :: uu_bdy_old,vv_bdy_old,S_bdy_old,T_bdy_old
#ifdef BFM_GOTM
   integer         :: bio_bs=0
   integer         :: ib,j,k
   REALTYPE        :: rat
...
#endif
!LEVEL1 'do_3d_bdy_ncdf, loop,t1,t2',loop,t1,t2
...
!        Note (KK): in principle the average can be calculated only once
...
#ifdef BFM_GOTM
      rat=((day-1)*secsprday+secondsofday)/t2
      ...
      elseif ( bio_calc) then
        bio_bs=0
        do ib=1,n_bio
!LEVEL1 'ncdf_3d_bdy, ib',ib
         if ( cc3d_id(ib) > 0 ) then
            bio_bs=bio_bs+1 
!LEVEL1 'ncdf_3d_bdy, bio_bs',bio_bs
            cc3d_bdy(bio_bs,:,:)=(1.-rat)*0.5*(cc3d_bdy_clim(bio_bs,prev,:,:)  &
                                  +cc3d_bdy_clim(bio_bs,this,:,:))   &
                      +rat*0.5*(cc3d_bdy_clim(bio_bs,next,:,:) &
                               +cc3d_bdy_clim(bio_bs,this,:,:))
!LEVEL1 'cc3d_bdy_clim',cc3d_bdy_clim(bio_bs,this,24,:)
!LEVEL1 'cc3d_bdy',cc3d_bdy(bio_bs,24,:)
         endif
!if (bio_bs.eq.3) stop
        enddo
!stop
...
!LEVEL1 'after read:  cc3d_bdy_wrk',cc3d_bdy_wrk(1,24,1:20)
...
!LEVEL1 'before switch'
!if (loop.eq.30) stop
#ifdef BFM_GOTM
      if ( bio_calc) then
        bio_bs=0
        do ib=1,n_bio
          if ( cc3d_id(ib) > 0 ) bio_bs=bio_bs+1 
        enddo
!LEVEL1 'bio_bs,kmax,nsbv',bio_bs,kmax,nsbv
!if (loop.eq.30) stop
        forall(i=1:bio_bs,j=1:kmax,k=1:nsbv) 
          cc3d_bdy_old(i,j,k) = cc3d_bdy_new(i,j,k)  
          cc3d_bdy_new(i,j,k) = cc3d_bdy_wrk(i,j,k)     !read_3d_bdy_data_ncdf reads into cc3d_bdy_wrk
        end forall
!LEVEL1 'after first block'
!if (loop.eq.30) stop
!        if (first) then
!          forall(i=1:bio_bs,j=0:kmax,k=1:nsbv) 
!            cc3d_bdy_old(i,j,k) = cc3d_bdy_wrk(i,j,k)  
!          end forall
!        endif
        forall(i=1:bio_bs,j=1:kmax,k=1:nsbv,cc3d_bdy_new(i,j,k)<_ZERO_) 
...
!LEVEL1 'before ifdef cc3d_bdy',cc3d_bdy(3,24,1:8)
!LEVEL1 'before interpolation'
!if (loop.eq.30) stop
#ifdef BFM_GOTM
      if ( bio_calc .and. .not. climatology) then
!LEVEL1 'interpolating cc3d_bdy, bio_bs',bio_bs
        forall(i=1:bio_bs,j=1:kmax,k=1:nsbv) 
          cc3d_bdy(i,j,k) = cc3d_bdy_old(i,j,k) + (cc3d_bdy_new(i,j,k) &
                                       - cc3d_bdy_old(i,j,k))*(t-t1)/(t2-t1)
      !cc3d_bdy is interpolated at read time, so no need for that here.
         end forall
!LEVEL1 'varname: ',var_names(3)
!LEVEL1 'do_3d_bdy_ncdf cc3d_bdy',cc3d_bdy(1,24,1:20)
!LEVEL1 '  cc3d_bdy_old',cc3d_bdy_old(1,24,1:20)
!LEVEL1 '  cc3d_bdy_new',cc3d_bdy_new(1,24,1:20)
!LEVEL1 '  cc3d_bdy_wrk',cc3d_bdy_wrk(1,24,1:20)
!LEVEL1 'nsbv,nsbvl',nsbv,nsbvl
!LEVEL1 't,t1,t2',t,t1,t2
!stop
      end if
#endif
!LEVEL1 'after ifdef cc3d_bdy',cc3d_bdy(3,24,1:8)
!LEVEL1 'end do_3d_bdy_ncdf'
!if (loop.eq.30) stop
#ifdef DEBUG
...
!                                           start,edges)
!             if (err .ne. NF90_NOERR) go to 10
!             cc3d_bdy_wrk(bio_bst,:,:)=cc3d_bdy_tmp
             err = nf90_get_var(ncid,cc3d_id(ib),wrk,  &
                                           start,edges)
             if (err .ne. NF90_NOERR) go to 10
             call interpolate_3d_bdy_ncdf(nsbvl,zax_len,wrk,H,kmax,hn,cc3d_bdy_tmp)
             cc3d_bdy_wrk(bio_bst,:,start(2):(start(2)+edges(2)-1))=cc3d_bdy_tmp
!if (ib.eq.3) then
!LEVEL1 'read_3d_bdy_data.ncdf, bio_bst',bio_bst
!LEVEL1 'start',start
!LEVEL1 'edges',edges
!LEVEL1 'wrk25',wrk(24,25)
!LEVEL1 'wrk31',wrk(24,31)
!LEVEL1 'cc3d_bdy_tmp',cc3d_bdy_tmp(24,1:20)
!LEVEL1 '  cc3d_bdy_wrk',cc3d_bdy_wrk(bio_bst,24,1:20)
!endif
!             call interpol(zax_len,zlev,wrk,H(i,j),kmax,hn(i,j,:), &
!                             cc3d_bdy_wrk(bio_bs,:,k))
!             cc3d_bdy_wrk(bio_bst,:,:)=wrk
           endif
         enddo
#endif
...
#endif
!LEVEL1 'read_3d_bdy_climatology_ncdf, bio_calc',bio_calc
!stop
...
#ifdef BFM_GOTM
   allocate(wrk(edges(1),edges(2)),stat=err) !AN
   if (err /= 0) stop 'ncdf_3d_bdy: Error allocating memory (wrk)' !AN
   wrk = _ZERO_  !AN
#endif
...
!LEVEL1 'ib,cc3d_id(ib)',ib,cc3d_id(ib)
                   if ( cc3d_id(ib) > 0 ) then
                     ...
                     if (err .ne. NF90_NOERR) go to 10
                     call interpol(zax_len,zlev,wrk,H(i,j),kmax,hn(i,j,:), &
                             cc3d_bdy_clim(bio_bs,m,:,k))
!LEVEL1 'ib,bio_bs,start,edges',ib,bio_bs,start,edges
!LEVEL1 'cc3d_id,wrk',cc3d_id(ib),wrk
!stop
                   endif
                 enddo
               endif
