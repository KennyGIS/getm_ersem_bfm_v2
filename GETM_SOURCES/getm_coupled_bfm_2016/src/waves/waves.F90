...
!
! !INTERFACE:
   subroutine uv_waves(veluEuler,velvEuler,UStokes,VStokes,UStokesC,VStokesC,Dvel,DU,DV,UEx,VEx)

   IMPLICIT NONE
!
! !INPUT PARAMETERS:
   REALTYPE,dimension(E2DFIELD),intent(in)    :: veluEuler,velvEuler
...
!BOC
   ...
   select case(waves_method)
      ...
      case (WAVES_VF)
         call vortex_force(veluEuler,velvEuler,UStokes,VStokes,UStokesC,VStokesC,DU,DV,UEx,VEx)
   end select
...
!
! !INTERFACE:
   subroutine uv_waves_3d(veluEuler3d,velvEuler3d,Dveln,hvel,hun,hvn,uuEx,vvEx)

   IMPLICIT NONE
!
! !INPUT PARAMETERS:
   REALTYPE,dimension(I2DFIELD),intent(in)    :: Dveln
   REALTYPE,dimension(I3DFIELD),intent(in)    :: veluEuler3d,velvEuler3d,hvel,hun,hvn
!
! !INPUT/OUTPUT PARAMETERS:
...
!BOC
   ...
   select case(waves_method)
      ...
      case (WAVES_VF)
         call vortex_force_3d(veluEuler3d,velvEuler3d,hun,hvn,uuEx,vvEx)
   end select
...
!BOC
   ...
   if (waveT.gt._ZERO_ .and. kD.lt.kD_deepthresh) then
      Hrms = sqrthalf * waveH ! equivalent monochromatic wave height in Uchiyama et al. (2010)
