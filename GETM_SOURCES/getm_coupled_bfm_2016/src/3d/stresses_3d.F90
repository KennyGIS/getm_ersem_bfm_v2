...
! !USES:
   ...
   use variables_3d, only: kumin,kvmin,velu3d=>veluEuler3d,velv3d=>velvEuler3d,hun,hvn,rru,rrv
...
!        KK-TODO: replace with semi-implicit calculation from stop_macro
         ...
         taubx(i,j) = - velu3d(i,j,k) * rru(i,j)      ! momentum flux
...
!        KK-TODO: replace with semi-implicit calculation from stop_macro
         ...
         tauby(i,j) = - velv3d(i,j,k) * rrv(i,j)      ! momentum flux
...
!        KK-TODO: check calculation
         taub(i,j)=sqrt(_HALF_*(                                      &
                   (velu3d(i-1,j  ,ku1)*rru(i-1,j  ))**2 &
                  +(velu3d(i  ,j  ,ku2)*rru(i  ,j  ))**2 &
                  +(velv3d(i  ,j-1,kv1)*rrv(i  ,j-1))**2 &
                  +(velv3d(i  ,j  ,kv2)*rrv(i  ,j  ))**2))
