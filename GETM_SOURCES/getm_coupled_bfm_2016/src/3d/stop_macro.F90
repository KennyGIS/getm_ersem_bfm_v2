...
! !USES:
   ...
   use variables_3d, only: veluAdv,velvAdv,veluEulerAdv,velvEulerAdv,veluAdvf,velvAdvf
   ...
   use m2d, only: interface_velocities
...
!             pressure contributes to slow terms
   if (kmax .gt. 1) then
      ...
      call interface_velocities(UEulerAdv,VEulerAdv,Dun,Dvn,veluEulerAdv,velvEulerAdv)
      if (waveforcing_method .ne. NO_WAVES) call interface_velocities(Uadv,Vadv,Dun,Dvn,veluAdv,velvAdv)

      call bottom_friction(veluEulerAdv,velvEulerAdv,Dun,Dvn,Dveln,ru,rv)
      call uv_advect(veluAdvf,velvAdvf,Uadv,Vadv,Dn,Dveln,Dun,Dvn)
      call uv_diffusion(0,veluEulerAdv,velvEulerAdv,Uadv,Vadv,Dn,Dun,Dvn) ! Has to be called after uv_advect.
      if (waveforcing_method .ne. NO_WAVES) then
         call uv_waves(veluEulerAdv,velvEulerAdv,Uadv-UEulerAdv,VAdv-VEulerAdv,UStokesCadv,VStokesCadv,Dveln,Dun,Dvn,UEx,VEx) ! add wave forcing
      end if
