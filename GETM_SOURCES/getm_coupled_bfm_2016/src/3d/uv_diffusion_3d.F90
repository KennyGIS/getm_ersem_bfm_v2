...
! !USES:
   ...
   use variables_3d, only: velu3d=>veluEuler3d,velv3d=>velvEuler3d,uu,vv,uuEx,vvEx,hn,hun,hvn
...
!BOC
   ...
   select case(Am_method)
      case(AM_LAPLACE)
         do k=1,kmax
            call uv_diff_2dh(0,uuEx(:,:,k),vvEx(:,:,k),velu3d(:,:,k),velv3d(:,:,k), &
                             uu(:,:,k),vv(:,:,k),hn(:,:,k),hun(:,:,k),hvn(:,:,k),   &
                             dudxC=dudxC_3d(:,:,k),                             &
#ifndef SLICE_MODEL
                             dvdyC=dvdyC_3d(:,:,k),                             &
#endif
