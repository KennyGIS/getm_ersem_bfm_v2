...
#ifdef GETM_BIO
#ifdef BFM_GOTM
!JM      call init_getm_bio(NAMLST2,trim(input_dir) // 'getm_bio.inp') !JM add dummy arg needed for gotm but not for getm
      call init_getm_bio(NAMLST2,trim(input_dir) // 'getm_bio.inp',0)
#else
...
#ifdef _NEW_GOTM_
   default_coordinate_xtype = NF90_DOUBLE
#ifdef _NCDF_SAVE_DOUBLE_
   default_xtype = NF90_DOUBLE
#endif
#endif
...
!BOC
   ...
   if (.not. dryrun) then
#ifdef _NEW_GOTM_
      call output_manager_start(julianday, secondsofday, 0, MinN-1)
#endif
