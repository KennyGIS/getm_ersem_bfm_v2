...
! !PUBLIC DATA MEMBERS:
   ...
   integer,public,parameter           :: HSIMT=12
   ...
   character(len=64),public,parameter :: adv_schemes(0:13) = &
      (/"advection disabled                             ",   &
        ...
        "advection disabled                             ",   &
        "advection disabled                             ",   &
        "TVD-HSIMT (third-order, monotone)              ",   &
        "SPL-max-1/3                                    "/)
!
! !LOCAL VARIABLES:
#ifdef STATIC
...
!-----------------------------------------------------------------------
   ...
   interface
      subroutine adv_split_u(dt,f,fi,Di,adv,U,DU,   &
         ...
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in) :: ffluxu
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in) :: nvd
      ...
      subroutine adv_split_v(dt,f,fi,Di,adv,V,DV,   &
         ...
         logical,dimension(E2DFIELD),intent(in)                       :: mask_update
         REALTYPE,dimension(E2DFIELD),intent(inout)                   :: fi,Di,adv
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in)        :: ffluxv
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in)        :: nvd
...
#ifdef _POINTER_REMAP_
   adv_gridU%mask_uflux(imin-HALO-1:,jmin-HALO:) => mask_updateH
#else
   allocate(adv_gridU%mask_uflux(-1+_IRANGE_HALO_-1,_JRANGE_HALO_),stat=rc)    ! work array
   if (rc /= 0) stop 'init_advection: Error allocating memory (mask_ufluxU)'
   adv_gridU%mask_uflux = mask_updateH
#endif
   adv_gridU%mask_vflux    => mask_xflux(_IRANGE_HALO_,_JRANGE_HALO_-1)
#ifdef _POINTER_REMAP_
   adv_gridU%mask_xflux(imin-HALO-1:,jmin-HALO:) => mask_vflux
#else
   allocate(adv_gridU%mask_xflux(-1+_IRANGE_HALO_-1,_JRANGE_HALO_),stat=rc)    ! work array
   if (rc /= 0) stop 'init_advection: Error allocating memory (mask_xfluxU)'
   adv_gridU%mask_xflux = mask_vflux
#endif
...
#ifdef _POINTER_REMAP_
   adv_gridU%dxu(imin-HALO-1:,jmin-HALO:) => dxc
   adv_gridU%dyu(imin-HALO-1:,jmin-HALO:) => dyc
#else
   allocate(adv_gridU%dxu(-1+_IRANGE_HALO_-1,_JRANGE_HALO_),stat=rc)    ! work array
   if (rc /= 0) stop 'init_advection: Error allocating memory (dxuU)'
   allocate(adv_gridU%dyu(-1+_IRANGE_HALO_-1,_JRANGE_HALO_),stat=rc)    ! work array
...
! !OUTPUT PARAMETERS:
   ...
   REALTYPE,dimension(:,:),pointer,contiguous,intent(in),optional :: nvd
!
! !LOCAL VARIABLES:
   type(t_adv_grid),pointer                   :: adv_grid
   REALTYPE,dimension(E2DFIELD),target        :: fi,Di,adv
   REALTYPE,dimension(:,:),pointer,contiguous :: p_Di,p_adv,p_nvd
   REALTYPE,dimension(:,:),pointer,contiguous :: p_ffluxu,p_ffluxv
   integer                                    :: j
!
!EOP
!-----------------------------------------------------------------------
      ...
      select case (split)
         ...
         case(NOSPLIT)
            ...
            select case (scheme)
               ...
               case((UPSTREAM),(P2),(SUPERBEE),(MUSCL),(P2_PDM),(HSIMT),(SPLMAX13))
...
!BOC
   ...
   select case (scheme)
      case((NOADV),(UPSTREAM),(UPSTREAM_2DH),(P2),(SUPERBEE),(MUSCL),(P2_PDM),(J7),(FCT),(P2_2DH),(HSIMT),(SPLMAX13))
      case default
...
!
! !LOCAL VARIABLES:
   REALTYPE           :: ratio,limiter,kappa,x,deltaf,deltafu
...
!BOC
   ...
   if (deltaf*deltafu .gt. _ZERO_) then
      ...
      ratio = deltafu / deltaf   ! slope ratio > 0

      select case (scheme)
         case (P2_PDM)
            x = one6th*(_ONE_-_TWO_*cfl)
            limiter = (_HALF_+x) + (_HALF_-x)*ratio ! always >= 1/3
            limiter = min(_TWO_*ratio/(cfl+1.d-10),limiter,_TWO_/(_ONE_-cfl))
         case (SUPERBEE)
            !limiter = max( _ONE_ , ratio )
            !limiter = min( _TWO_*ratio , limiter , _TWO_ )
!           this requires only 3 instead of 4 comparisons
            limiter = max(min(_TWO_*ratio,_ONE_),min(ratio,_TWO_))
         case (SPLMAX13)
            limiter = one3rd*max(_ONE_+_TWO_*ratio,_TWO_+ratio) ! always > 2/3
            limiter = min(_TWO_*ratio,limiter,_TWO_)
         case (HSIMT) ! Wu and Zhu (2010, OCEMOD)
            kappa = _ONE_ - cfl
            x = _QUART_*( kappa - one3rd/kappa )
            limiter = (_HALF_+x) + (_HALF_-x)*ratio ! can become negative!!!
            limiter = max( _ZERO_ , limiter )
            limiter = min(_TWO_*ratio,limiter,_TWO_)
         case (MUSCL)
