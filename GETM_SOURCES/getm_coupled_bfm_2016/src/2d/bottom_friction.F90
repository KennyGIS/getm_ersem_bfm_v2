...
!  !LOCAL VARIABLES:
   ...
   REALTYPE,dimension(:,:),allocatable,save :: vel_U,vel_V
...
!BOC
   ...
   if (bottfric_method .eq. 0) return
      ...
      if (bottfric_method.eq.2 .or. bottfric_method.eq.3) then
         allocate(vel_U(E2DFIELD),stat=rc)
         if (rc /= 0) stop 'init_2d: Error allocating memory (vel_U)'
         vel_U=_ZERO_

         allocate(vel_V(E2DFIELD),stat=rc)
         if (rc /= 0) stop 'init_2d: Error allocating memory (vel_V)'
         vel_V=_ZERO_
      end if
...
!        calculate velvC
         do i=imin-HALO,imax+HALO
            if (az(i,j) .ge. 1) then
               work2d(i,j) = _HALF_ * ( velv(i,j-1) + velv(i,j) )
            end if
         end do
#ifdef SLICE_MODEL
!$OMP END DO
!$OMP DO SCHEDULE(RUNTIME)
         ...
         do i=imin-HALO,imax+HALO-1
            if ( au(i,j) .ge. 1 ) then
               ...
               vel_U(i,j) = sqrt( velu(i,j)*velu(i,j) + work*work )
               z0d = zub0(i,j)
!              Note (KK): note shifting of log profile so that U(-H)=0
               sqrtcd = kappa / log( _ONE_ + _HALF_*DU(i,j)/z0d )
               if (avmmol.gt._ZERO_ .and. vel_U(i,j).gt._ZERO_) then
                  do it=1,z0d_iters
                     z0d = zub0(i,j) + _TENTH_*avmmol/(sqrtcd*vel_U(i,j))
...
!$OMP DO SCHEDULE(RUNTIME)
      ...
      do j=jmin-HALO,jmax+HALO
         ...
         do i=imin-HALO+1,imax+HALO-1
            if (az(i,j) .ge. 1) then
               work2d(i,j) = _HALF_ * ( velu(i-1,j) + velu(i,j) )
            end if
         end do
#ifndef SLICE_MODEL
      end do
      ...
      do j=jmin-HALO,jmax+HALO-1
         ...
         do i=imin-HALO+1,imax+HALO-1
            if ( av(i,j) .ge. 1 ) then
               ...
               vel_V(i,j) = sqrt( work*work + velv(i,j)*velv(i,j) )
               z0d = zvb0(i,j)
!              Note (KK): note shifting of log profile so that V(-H)=0
               sqrtcd = kappa / log( _ONE_ + _HALF_*DV(i,j)/z0d )
               if (avmmol.gt._ZERO_ .and. vel_V(i,j).gt._ZERO_) then
                  do it=1,z0d_iters
                     z0d = zvb0(i,j) + _TENTH_*avmmol/(sqrtcd*vel_V(i,j))
...
!$OMP END PARALLEL
   ...
   if (waveforcing_method .ne. NO_WAVES) then
      if (bottfric_method.eq.2 .or. bottfric_method.eq.3) then
         ...
         call bottom_friction_waves(velu,velv,DU,DV,Dvel,vel_U,vel_V,ru,rv,p_zub,p_zvb,p_taubmax)
         call tic(TIM_BOTTFRIC)
      end if
   end if
