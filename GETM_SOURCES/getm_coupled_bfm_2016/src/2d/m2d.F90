...
! !USES:
   ...
   use domain, only: dry_z, dry_u, dry_v
!KB   use get_field, only: get_2d_field
   ...
   interface
      ...
      subroutine uv_diffusion(An_method,velu,velv,U,V,D,DU,DV,phydis)
         use domain, only: imin,imax,jmin,jmax
         IMPLICIT NONE
         integer,intent(in)                      :: An_method
         REALTYPE,dimension(E2DFIELD),intent(in) :: velu,velv,U,V,D,DU,DV
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in),optional :: phydis
      end subroutine uv_diffusion

      subroutine uv_diff_2dh(An_method,UEx,VEx,velu,velv,U,V,D,DU,DV,  &
         ...
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in),optional :: dudyX,dvdxX
         ...
         REALTYPE,dimension(:,:),pointer,contiguous,intent(in) :: phydis
         REALTYPE,dimension(E2DFIELD),intent(out),optional :: hsd_u,hsd_v
      end subroutine uv_diff_2dh

      subroutine bottom_friction(velu,velv,DU,DV,Dvel,ru,rv,kwe,zub,zvb,taubmax)
         use domain, only: imin,imax,jmin,jmax
         IMPLICIT NONE
         REALTYPE,dimension(E2DFIELD),intent(in)           :: velu,velv,DU,DV,Dvel
...
! !LOCAL VARIABLES:
   ...
   namelist /m2d/ &
          ...
          residual,sealevel_check,depth_check,                 &
...
!  Note (KK): sse=0,U=V=0
   ...
   if (check_cfl .and. .not. rigid_lid) call cfl_check()
#endif
...
!              or initialise()
      call depth_update(zo,z,D,Dvel,DU,DV,loop=0)
   end if
...
!  KK-TODO: velocity update *after* reset of U and V ?!
   call depth_update(zo,z,D,Dvel,DU,DV,loop=depth_check)
...
!BOC
   ...
   call interface_velocities(UEuler,VEuler,DU,DV,veluEuler,velvEuler)
   if (waveforcing_method .ne. NO_WAVES) call interface_velocities(U,V,DU,DV,velu,velv)

   if (mod(loop-1,MM) .eq. 0) then        ! MacroMicro time step
      call bottom_friction(veluEuler,velvEuler,DU,DV,Dvel,ru,rv,taubmax=taubmax)
   end if

   call uv_advect(veluf,velvf,U,V,D,Dvel,DU,DV,numdis_2d)
   call uv_diffusion(An_method,veluEuler,velvEuler,U,V,D,DU,DV,phydis_2d) ! Has to be called after uv_advect.
   call toc(TIM_INTEGR2D)

   if (waveforcing_method .ne. NO_WAVES) then
      call uv_waves(veluEuler,velvEuler,UStokes,VStokes,UStokesC,VStokesC,Dvel,DU,DV,UEx,VEx) ! add forcing
      call stokes_drift(dtm,Dvel,UEx,VEx)             ! calculate new Stokes drift
   end if

   call tic(TIM_INTEGR2D)
   call dryfac_update( D, DU, DV, dry_z, dry_u, dry_v)
   call toc(TIM_INTEGR2D)
...
!                that fulfill dxU+dyV=0
   ...
   if (.not. rigid_lid) then
      call sealevel(loop)
      call tic(TIM_INTEGR2D)
      call depth_update(zo,z,D,Dvel,DU,DV,loop=loop)
      call toc(TIM_INTEGR2D)
   end if
