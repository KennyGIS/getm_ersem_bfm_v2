...
#ifdef BFM_GOTM
   use airsea_driver,      only: wx_wind=>u10,wy_wind=>v10,daylength    
   use bio,only:init_var_bio,init_bio,do_bio,end_bio,bio_calc, &
       test_ben_on_zero_depth,get_bio_updates
!JM   use bio_bfm,only: set_pointers_from_gotm_to_bfm,set_env_bio_bfm,test_structure !AN using Piet's version
   use bio_bfm,only: set_env_bio_bfm,test_structure !JM 2016 version
!AN   use bio_bfm,only: set_pointersto_bio_bfm,set_env_bio_bfm,test_structure
   use bfm_output,only: reset_count
   use bfm_output,only:setup_bio_output,prepare_bio_output
   use bfm_output_ncdf,only:bio_save !AN names changed see line below
   !use bfm_ncdf_output,only:bio_1d_save
...
#endif
#ifdef BFM_GOTM
   integer, parameter                  :: unit_bio=64,unit_bfm=65
#endif
!
! !REVISION HISTORY:
...
! !LOCAL VARIABLES:
   ...
   namelist /model_setup/ title,nlev,dt,restart_offline,restart_allow_missing_variable, &
                          restart_allow_perpetual,cnpar,buoy_method, sw_hotstart
   ...
   namelist /output/      out_fmt,out_dir,out_fn,nsave,diagnostics,     &
                          diff_k,Ri_crit,rad_corr
#endif
  character(len=1000) :: line !AN  
...
#ifdef BFM_GOTM
!LEVEL1 'call init_output'
!stop
      call init_output(title,nlev,latitude,longitude)
!LEVEL1 'call init_bio'
!stop
...
!AN     call set_pointersto_bio_bfm(nlev,dt,h,t,s,rho,nuh,u,v)
!JM      call set_pointers_from_gotm_to_bfm(1,nlev,dt,h,t,s,rho,nuh) !From Piet BFM

!AN      call set_env_bio_bfm(depth0,u_taub,daylength,&
!AN          wx_wind%value,wy_wind%value,I_0%value,julianday,rscalar)
!JM      call set_env_bio_bfm(nlev,depth0,u_taub,daylength,&   !AN Piet's version
!JM                        wx_wind%value,wy_wind%value,u(1),v(1),i_0%value,julianday,rscalar)
!LEVEL1 'call init_bio'
!LEVEL1 'i_0%value',i_0%value
!LEVEL1 'I_0%value',I_0%value
!stop
      call set_env_bio_bfm(nlev,dt,depth0,u_taub,h,t,s,rho,nuh,daylength,&   !JM 2016 version
                        wx_wind%value,wy_wind%value,u(1),v(1),i_0%value,julianday,rscalar)


!AN      call init_bfm(namlst,unit_bfm)
!JM      call init_var_bio(namlst,unit_bfm,nsave)
!LEVEL1 'call init_var'
!stop
      call init_var_bio(namlst,unit_bfm)   !JM 2016 version
      call prepare_bio_output(0)
!LEVEL1 'after prepare_bio_output(0) '
!stop
#endif
...
94 FATAL 'I could not read the "output" namelist'
   backspace(namlst) !AN
   read(namlst,fmt='(A)') line !AN
   LEVEL2 'Invalid line in namelist: '//trim(line) !AN 
...
!BOC
   ...
   i=0
   call hotstart_read(max(0,sw_hotstart))
   do n=MinN,MaxN
...
#ifdef BFM_GOTM
!LEVEL1 'call prepare_output'
!stop
      call prepare_output(int(n,4))
#endif
...
!JM      call set_env_bio_bfm(nlev,depth0,u_taub,daylength,&   !an piet's version
!JM                        wx_wind%value,wy_wind%value,u(1),v(1),i_0%value,julianday,rscalar)
!LEVEL1 'second call set_env_bio_bfm, I_0:',I_0%value
!stop
      call set_env_bio_bfm(nlev,dt,depth0,u_taub,h,t,s,rho,nuh,daylength,&   !JM 2016 version
                        wx_wind%value,wy_wind%value,u(1),v(1),i_0%value,julianday,rscalar)
      call get_bio_updates(nlev,bioshade)
!JM      call do_bio(dt,nlev,write_results)
!LEVEL1 'gotm: do_bio'
!stop
      call do_bio(write_results)  !JM 2016 version
!LEVEL1 'gotm: after do_bio'
!stop
#endif
...
#ifdef BFM_GOTM
!LEVEL1 'calling do_output'
!stop
      ! store GOTM output to bfm output file
      call do_output(int(n,4),nlev)
!JM      if (write_results) then
!LEVEL1 'prepare10'
!'stop
      ! averaging variables that need to be averaged
...
!LEVEL1 'prepare10'
   ...
   STDERR LINE
   if (sw_hotstart>=0)call hotstart_save(-1)
   return
